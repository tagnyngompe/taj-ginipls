#TO RUN : python -m ginipls.models.hyperparameters
import numpy as np
import traceback #for exception
import itertools # for cartesian product of params ranges
from sklearn.model_selection import KFold
import logging
logger = logging.getLogger(__name__)

## DIY : sklearn.kfold + for(nu_range) + fit + score
def select_hyperparameters_with_cross_val(cls, X, y, nu_range, n_components_range, n_folds, best_expectable_score=1.):
  """"""
  kf = KFold(n_splits=n_folds, shuffle=True)
  kf.get_n_splits(X)
  best_mean_score = 0.
  best_nu_ = 0.
  best_n_comp_ = 0.
  for nu_, n_comp_ in itertools.product(nu_range, n_components_range):
  params = {'nu' : nu_; 'n_components': n_comp_}
  mean_score = 0.
  fold_id = 0
  n_valid_folds = 0
  for train_index, test_index in kf.split(X):
    try:        
      logger.debug("TRAIN:", train_index, "TEST:", test_index)
      X_train, X_test = X[train_index], X[test_index]
      y_train, y_test = y[train_index], y[test_index]
      cls.set_params(**params)
      cls.fit(X_train.tolist(), y_train.tolist())
      logger.debug(gpls.get_params())
      fold_score = gpls.score(X_test.tolist(), y_test.tolist())        
      logger.debug("fold_%d_score(nu==%.3f) = %.3f" % (fold_id, nu_,fold_score))
      mean_score += fold_score
    except Exception:
      tb = traceback.format_exc()
      logger.error("ERROR for nu_==%.3f, n_comp_==%d [fold %d]" % (nu_,n_comp_, fold_id), tb)
    else:
      n_valid_folds += 1
    finally:
      fold_id += 1
  mean_score = mean_score / n_folds
  logger.debug("mean_score(nu==%.3f, n_comp_==%d,n_valid_folds=%d) = %.3f" % (nu_,n_comp_,n_valid_folds,mean_score))
  if best_mean_score < mean_score:
    best_mean_score = mean_score
    best_nu_ = nu_
    best_n_comp_ = n_comp_
  if best_mean_score == best_expectable_score: #stop if the max score is reached
    break 
  logger.info("best_mean_score = %.3f (with nu_==%.3f & n_comp_==%d)" % (best_mean_score, best_nu_, best_n_comp_))
  return {}
  

def 

if __name__ == "__main__":
  X=[[2.0, 0.0, 7.0, 4, 5.2, 9.7], [3.0, 1.0, 5.0, 4, 1.0, .97], [0.0, 4.0, 5.0, 4, .1235, 2.58], [4.0, 0.0, 7.0, 4, 10, 4.78], [4.0, 1.0, 8.0, 4, 1, 5], [1.5, 1.3, 1.1, 4, 7, 6]]
  y=[0, 0, 0, 1, 1, 1]
  from ginipls.models.ginipls import PLS, PLS_VARIANT
  nu_min = 1
  nu_max = 3
  nu_step = 0.1
  nu_range = [i*nu_step for i in range(int(nu_min/nu_step),int(nu_max/nu_step))]
  print('nu_range =', nu_range)

  n_components_min = 1
  n_components_max = len(X[0]) # nb de caractéristiques
  n_components_step = 1
  n_components_range = range(n_components_min, n_components_max, n_components_step)
  print('n_components_range =', n_components_range)
  